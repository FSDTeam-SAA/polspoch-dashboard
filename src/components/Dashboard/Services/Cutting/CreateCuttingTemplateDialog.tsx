...
import { useCreateCuttingTemplate } from "@/lib/hooks/useCuttingServices";
...
interface CreateCuttingFormValues {
  templateId: string;
  shapeName: string;
  cuts: number;
  materials: {
...
export function CreateCuttingTemplateDialog({
  trigger,
}: {
  ...
  const { register, control, handleSubmit, reset, getValues, formState } =
    useForm<CreateCuttingFormValues>({
      defaultValues: {
        templateId: "",
        shapeName: "",
        cuts: 0,
  ...
  const { mutate: createTemplate, isPending } = useCreateCuttingTemplate();
  ...
  const onSubmit = (data: CreateCuttingFormValues) => {
    if (!data.image?.[0]) {
    ...
    createTemplate(payload, {
      onSuccess: () => {
        toast.success("Cutting template created successfully");
        setIsOpen(false);
        reset();
        setImagePreview(null);
      },
      ...
      <DialogTrigger asChild>
        {trigger || (
          <Button className="bg-[#7E1800] hover:bg-[#7E1800]/90 text-white">
            ...
            Add Cutting Template
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Create Cutting Template</DialogTitle>
          <DialogDescription>
            Add a new cutting shape template to the system.
          </DialogDescription>
        </DialogHeader>
